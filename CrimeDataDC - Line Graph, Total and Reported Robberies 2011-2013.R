...
Number <- daFile$RobsReported
TotNumber <- daFile$NumReported

Month
Number

data.set <- data.frame(Month,Number,TotNumber)
data.set

ggplot(data = data.set, aes(x=Month, y = Number, group=1))+ 
geom_line(colour="#339966", size=2)+
theme(axis.text.x = element_text(angle = 65, hjust = 1))+
xlab("Month")+
ylab("Number of Reported Incidents")+
ggtitle("Number of Robberies Reported, 2011-2013")+
